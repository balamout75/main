...
public class TableAdapter extends UnicastRemoteObject implements TableAdapterInterface , Serializable {
    ...
    public TableAdapter(DataModuleInterface aOwner,INIACRMIInterface aServer) throws RemoteException {
        ...
        System.out.println("РЎРґРµР»Р°Р»Рё  SQL Р·Р°РїСЂРѕСЃ");
      }

    public void executeQuery() {
        try {
            System.err.println("TableAdapter - Р�СЃРїРѕР»РЅСЏСЋ Р·Р°РїСЂРѕСЃ "+SQLSelectString);
            SQLSelectString = Owner.makeSelectSQL();
            ...
            if (E.getErrorCode()==904) {
                try {
                    ...
                    System.out.println("Р”РѕР±Р°РІР»СЏРµРј СЃС‚РѕР»Р±РµС† "+AlterSQL);
                    statement.execute(AlterSQL);
                    executeQuery();
                } catch (Exception ex) {
                    System.err.println("РћС€РёР±РєР° РїСЂРё РґРѕР±Р°РІР»РµРЅРёРё РєРѕР»РѕРЅРєРё "+ex);
                }
            }
        } catch (Exception ex) {
            System.err.println("РћС€РёР±РєР° РІ TableAdapter.executeQuery()"+ex);
        }
    }
    ...
    public Vector FirstRow() throws RemoteException {
        ...
        try {
            ...
            System.err.println("TableAdapter - РѕС€РёР±РєР° РІ РїРµСЂРІРѕР№ СЃС‚СЂРѕРєРµ "+Ex);
        }
    ...
    public Vector InsertRow() throws RemoteException {
        ...
        try {
            ...
            System.err.println("TableAdapter - РћС€РёР±РєР° РІ РґРѕР±Р°РІР»РµРЅРёРё СЃС‚СЂРѕРєРё "+Ex);
        }
        return Res;
    }       
    ...
    public String getColumnNames(int Column) throws RemoteException {
        ...
        try {
            ...
            System.err.println("РљРѕР»РѕРЅРєРё " + Column);
            System.err.println("Р“РґСЋРє РІ РєРѕР»СѓРјРЅРЅР°РјРµ" + e);
        }
        return Res;
    }
    ...
    public void updateTable(String Value, int FieldNum, int fixMode) throws RemoteException {
        try {
            ...
            if (fixMode==1) {
                ...
                System.err.println("РћС‚РІРµС‚ С„РёРєСЃРёСЂСѓРµРј " + query);
                statement.execute(query);
            }
            ((Vector)rows.get(CurrRow)).set(FieldNum,Value);
        } catch (Exception E){
            System.err.println("РћС€РёР±РєР° РїСЂРё РїСЂРѕРІРµРґРµРЅРёРё Р°РїРґРµР№С‚Р° "+E);
        }
    }
