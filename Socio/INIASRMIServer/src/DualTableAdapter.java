...
public class DualTableAdapter extends UnicastRemoteObject implements DualTableAdapterInterface , Serializable {
//public class TableAdapter extends UnicastRemoteObject {
    ...
    int                 Mode        =0;
    boolean             Clearence   =true;    
    int                 AllSumm     =0;
    Integer             Total       =0;
    
    

    public DualTableAdapter(DataModuleInterface aOwner,INIACRMIInterface aServer) throws RemoteException {
        Server=aServer;
    ...
    public void executeQuery() {
        try {
            ...
            String SeqSql="select count(*) FROM "+Owner.getTableName();
            ...
            if ((RQX==null)&(RQY==null)) {
                TypeOfTable=0;
                    ...
                    NameX = "Quest"+RQY.getID();
                    QuestBlock ="a.str1";
                    Questions.add(Q1);
                    ArrayList DimensiomHeads = new ArrayList();
                    int i = 0;
                    while (i<Q1.getSize()) {
                        HeadRecord CurrentHead = new HeadRecord(new Integer(i+1).toString());                        
                    ...
                    TypeOfTable=1;
                        ...
                        TypeOfTable=2;
                            ...
                            QuestBlock ="a.str1, b.str2";
                            BinY="CROSS JOIN (SELECT t2.row_id, regexp_substr(t2."+NameY+ 
                                 " , '[^ ]+', 1, LEVEL) str2 FROM (SELECT tt2.row_id, tt2."+NameY+" FROM "+Owner.getTableName()+
                                 " tt2) t2 CONNECT BY instr(TRIM(' ' FROM t2."+NameY+
                                 "), ' ', 1, LEVEL - 1) > 0 AND PRIOR t2.row_id = t2.row_id AND PRIOR dbms_random.value IS NOT NULL) b WHERE a.row_id=b.row_id ";
                            Questions.add(Q1);
                            ArrayList DimensiomHeads = new ArrayList();
                            int i = 0;
                            while (i<Q1.getSize()) {
            ...
            SQLStr=SQLStr+QuestBlock+",count(*) from (SELECT t1.row_id, regexp_substr(t1."+NameX+ 
                    " , '[^ ]+', 1, LEVEL) str1 FROM (SELECT tt1.row_id, tt1."+NameX+" FROM "+Owner.getTableName()+
                    " tt1) t1 CONNECT BY instr(TRIM(' ' FROM t1."+NameX+
                    "), ' ', 1, LEVEL - 1) > 0 AND PRIOR t1.row_id = t1.row_id AND PRIOR dbms_random.value IS NOT NULL) a "+BinY+
                    " Group by "+QuestBlock+" Order by "+QuestBlock;
    ...
    public Record getValidElement(ArrayList AL, String Index) {
        int i=0;
        Record Result=null;
        boolean SearchFlag=true;
        while ((i<AL.size())&SearchFlag) {
            Result=(Record)AL.get(i);
            SearchFlag=!Result.Name.equalsIgnoreCase(Index);
        i++;
        }
        if (SearchFlag) Result = null;
        return Result;
    }    
    ...
    class Record {
        ...
        public Record getRecord(int j) {
            Record Result=null;
            String Coord = new Integer(j).toString();
            return getValidElement(Branches,Coord);
        }
        
        public Record getRecord(String Str) {
            return getValidElement(Branches,Str);
        }
    ...
    public int getColumnCount() throws RemoteException{
        ...
        try {
            ...
            if (Clearence) {
                int i = 0;
                while (i<AL.size()) {
                    HeadRecord CH = (HeadRecord) AL.get(i);                                   
                    if (CH.Count!=0) Res ++;
                    i++;
                }
            } else Res=AL.size()+2;
    ...
    public String getHeaderCount(ArrayList RequestList,int Dimension)  throws RemoteException{
        ...
        if (Coord==0) {
            ResultStr="Всего";
                    ...
                    if (Dimension==2) {
                        if (Mode==1) ResultStr=Total.toString();
                        else ResultStr=new String("100%");       
                    }
               } else {
                    switch (Mode) {
                            case 1: {
                            ...
                            case 2: {
                                Integer znamen=Total;
                                //Integer znamen=new Integer(AllSumm);
                                HeadRecord HR = (HeadRecord)DimensionHead.get(Coord-1);
                                double Res=(double)HR.Count/znamen;
    ...
    public String getHeaderCount2(ArrayList RequestList,int Dimension)  throws RemoteException{
        int                 Coord           = ((Integer)RequestList.get(Dimension-1)).intValue();
        ArrayList           DimensionHead   = null;
        int Range = 0;
        try {
    ...
    public String doSurf(Record Branch, ArrayList RequestList,int Dimension)  throws RemoteException{
        ...
        if (Coord==0) {
            try {
                //ResultStr="РОРР";
                ResultStr=getHeader(RequestList,2);
            } catch (Exception ex) {
                ResultStr="#";
            }
        } else 
            if (Coord==(Range+1)) {
                try {
                    ResultStr=getHeaderCount(RequestList,2);
                } catch (Exception ex) {
                    ResultStr="#";
                }
            } else {
                //Record CurrentBranch=(Record)Branch.Branches.get(Coord-1);
                    HeadRecord HR=(HeadRecord)DimensionHead.get(Coord-1);
                    Record CurrentBranch=Branch.getRecord(HR.Name);
                    ResultStr=doSurf2(CurrentBranch, RequestList,Dimension+1);
                }
        return ResultStr;
    }
    
public String doSurf2(Record Branch, ArrayList RequestList,int Dimension)  throws RemoteException{
        String ResultStr="";
        int                 Coord           = ((Integer)RequestList.get(Dimension-1)).intValue();
        ArrayList           DimensionHead   = null;
        int                 Range           =0;
        try {
            DimensionHead   = (ArrayList)Headers.get(1);
            Range = DimensionHead.size();
        } catch (Exception ex) {
        }    
        if (Coord==0) {
            try {
                ResultStr=getHeader(RequestList,(1));
            } catch (Exception ex) {
                ResultStr="#";
            }
        } else 
            if (Coord==(Range+1)) {
                ...
                if (Dimension==2) {
                    ...
                    switch (Mode) {
                            ...
                            case 2: {
                                znamen=new Integer(getHeaderCount2(RequestList,(2)));
                                double Res=(double)CurrentBranch.Count/znamen;
                                ResultStr=myNumberFormat.format(Res);                                
                                break;
                            }
                            case 3: {
                                znamen=new Integer(getHeaderCount2(RequestList,(1)));
                                double Res=(double)CurrentBranch.Count/znamen;
                                ResultStr=myNumberFormat.format(Res);
                                break;
                            }
